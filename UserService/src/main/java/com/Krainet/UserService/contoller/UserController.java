...
import com.Krainet.UserService.service.NotificationService;
...
public class UserController {
    ...
    public UserDTO create(@RequestBody UserCreateDTO userCreateDTO) {
        ...
        // Отправляем уведомление администраторам только для обычных пользователей
        if ("USER".equals(user.getRole())) {
            notificationService.notifyAdminsUserCreated(user.getUsername(), user.getEmail());
        }
    ...
    @PutMapping("/users/{id}")
    @PreAuthorize("@userSecurityService.canAccessUser(authentication, #id)")
    @Operation(summary = "Обновить данные пользователя", description = "ВАЖНО: При запросах от имени администратора смотреть на изменения ROLE. По умолчанию ROLE поставлена на USER")
    @Parameters({
            @Parameter(name = "id", description = "ID пользователя", example = "2")
    })
    @ResponseStatus(HttpStatus.OK)
    public UserDTO update(
        ...
        // Отправляем уведомление администраторам только для обычных пользователей
        if ("USER".equals(user.getRole())) {
            notificationService.notifyAdminsUserUpdated(user.getUsername(), user.getEmail());
        }
    ...
    public ResponseEntity<Void> delete(@PathVariable Long id) {
        ...
        // Отправляем уведомление администраторам только для обычных пользователей
        if ("USER".equals(role)) {
            notificationService.notifyAdminsUserDeleted(username, email);
        }
